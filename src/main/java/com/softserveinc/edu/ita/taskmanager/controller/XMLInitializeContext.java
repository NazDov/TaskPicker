...
public class XMLInitializeContext {
    ...
    public XMLInitializeContext loadIntoCache() throws TaskObjectNotAbstractTaskException {
    ...
    private XMLInitializeContext loadToTaskFactoryCache(String idVal, String nameVal, String descrVal) throws TaskObjectNotAbstractTaskException {

        try {
            final Class<?> className = Class.forName(nameVal);


            if (className.getSuperclass() == AbstractTask.class) {

                TaskFactory.getTaskById(idVal, (Class<? extends AbstractTask>) className,
                        descrVal);

            } else {
                throw new TaskObjectNotAbstractTaskException(""+className);
            }
            return this;
